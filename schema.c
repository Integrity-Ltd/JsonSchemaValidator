...
void Schema::m_validate_object(const Ref & in,
  const Value & schema) {
...
void Schema::m_validate_string(const Ref & in,
  const Value & schema) {
...
void Schema::m_validate_number(const Ref & in,
  const Value & schema) {
...
void Schema::m_validate_integer(const Ref & in,
  const Value & schema) {
...
Schema::Schema(const Value & root, const Ref & in, const Value & schema, bool error)
  : m_schema(schema), m_go_through_errors(error), m_result(true), m_root(root) {
...
void Schema::set_error_type(bool go_through) {
  m_go_through_errors = go_through;
}
